...
Deno.serve(async (req) => {
	...
	try {
		...
		if (action === 'send-invite') {
			if (!inviteEmail || !inviteRole || !password)
				throw new Error('Please provide email, role, and your password.');

			// 1. Security check: User cannot invite a Developer
			if (inviteRole === 'developer') {
				throw new Error(
					'For security reasons, developer accounts cannot be created via invitations.'
				);
			}
		...
		if (action === 'update-role') {
			if (!userId || !newRole || !password)
				throw new Error('User ID, new role, and your password are required.');
		...
		if (action === 'delete-user') {
			if (!userId || !password)
				throw new Error('User ID and your confirmation password are required.');
