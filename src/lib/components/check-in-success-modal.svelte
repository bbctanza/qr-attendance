<script lang="ts">
	...
	let { isOpen = $bindable(), memberName, memberId, careGroup, time, onClose, autoCloseDuration = 5000 } = $props();
		...
		<div class="relative z-10 w-full max-w-sm rounded-3xl bg-card border border-border/40 shadow-2xl animate-in fade-in zoom-in duration-300 pointer-events-auto">
			...
			<div class="p-6 sm:p-8 space-y-6 relative">
				...
				<div class="bg-muted/40 rounded-2xl p-4 space-y-3">
					...
					<div class="h-px bg-border/40"></div>
					<div class="flex justify-between items-center">
						<span class="text-sm text-muted-foreground font-medium">Care Group:</span>
						<span class="font-semibold text-right">{careGroup}</span>
					</div>
					<div class="h-px bg-border/40"></div>
					<div class="flex justify-between items-center">
						<span class="text-sm text-muted-foreground font-medium">Time:</span>
						<span class="font-mono font-semibold text-right">{time}</span>
					</div>
				</div>
