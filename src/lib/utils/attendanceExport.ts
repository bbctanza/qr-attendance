 ...
 */
export function exportToCSV(records: ExportRecord[], options: ExportOptions): void {
	try {
		let csv = '';
		let filename = '';
 ...
 */
export async function exportToPDF(records: ExportRecord[], options: ExportOptions): Promise<void> {
	try {
		...
		doc.text(`Attendance ${viewType} Report`, doc.internal.pageSize.getWidth() / 2, 20, {
			align: 'center'
		});
		...
		doc.text(`Generated: ${exportDate}`, doc.internal.pageSize.getWidth() / 2, 36, {
			align: 'center'
		});
		doc.text(`Total Records: ${recordCount}`, doc.internal.pageSize.getWidth() / 2, 42, {
			align: 'center'
		});
		...
		doc.text(options.stats.totalPresent.toString(), startX + boxWidth / 2, startY + 16, {
			align: 'center'
		});
		...
		doc.setTextColor(102, 102, 102);
		doc.text('Total Absent', startX + boxWidth + spacing + boxWidth / 2, startY + 8, {
			align: 'center'
		});
		doc.setFontSize(14);
		doc.setTextColor(220, 38, 38); // Red
		doc.text(
			options.stats.totalAbsent.toString(),
		...
		doc.setTextColor(102, 102, 102);
		doc.text('Attendance Rate', startX + (boxWidth + spacing) * 2 + boxWidth / 2, startY + 8, {
			align: 'center'
		});
		doc.setFontSize(14);
		doc.setTextColor(37, 99, 235); // Blue
		doc.text(
			`${options.stats.attendanceRate}%`,
