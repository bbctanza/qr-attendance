...
<div class="space-y-6 p-6">
		...
		<Card>
			...
			<CardContent>
				...
				<p class="mt-1 text-xs text-muted-foreground">
					{auditEnabled ? 'Logging active' : 'Audit trail disabled (dev mode)'}
				</p>
			</CardContent>
		</Card>
			...
			<CardContent>
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
					<div>
						...
						<Input id="search" placeholder="Search..." bind:value={searchText} class="mt-1" />
					</div>
				...
				<Table>
					...
					<TableBody>
						{#each paginatedLogs as log (log.id)}
							<TableRow>
								...
								<TableCell>
									...
									<div class="mt-1 text-xs text-muted-foreground">{log.entity_id}</div>
								</TableCell>
								<TableCell>
								...
								<TableCell class="flex justify-end space-x-2 text-right">
									<Button variant="ghost" size="sm" onclick={() => toggleDetailView(log)}>
										<Eye class="h-4 w-4" />
									</Button>
						...
						{#each ghostRows as ghost (ghost.ghostId)}
							<TableRow class="pointer-events-none opacity-0">
								<TableCell>&nbsp;</TableCell>
								<TableCell>&nbsp;</TableCell>
								<TableCell>&nbsp;</TableCell>
								<TableCell>&nbsp;</TableCell>
					...
					{#if selectedLog}
						<div class="space-y-4">
							{#if selectedLog.change_diff}
								<div
									class="space-y-3 overflow-x-auto rounded-md bg-slate-50 p-4 font-mono text-xs sm:text-sm"
								>
	...
	<div class="mt-4 flex items-center justify-between">
		<p class="text-xs text-muted-foreground">
			Showing {Math.min((currentPage - 1) * itemsPerPage + 1, filteredLogs.length || 1)}–{Math.min(
				currentPage * itemsPerPage,
				filteredLogs.length
			)} of {filteredLogs.length} entries (Total: {auditLogs.length})
		</p>
	</div>
</div>
