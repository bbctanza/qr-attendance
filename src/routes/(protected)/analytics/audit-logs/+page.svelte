...
<div class="space-y-6 p-6">
    ...
    <div class="mt-4 flex justify-between items-center">
        <p class="text-xs text-muted-foreground">
            Showing {Math.min((currentPage - 1) * itemsPerPage + 1, filteredLogs.length || 1)}–{Math.min(currentPage * itemsPerPage, filteredLogs.length)} of {filteredLogs.length} entries (Total: {auditLogs.length})
        </p>
    </div>
</div>
