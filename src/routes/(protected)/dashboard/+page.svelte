    ...
    async function fetchDashboardData() {
        try {
            ...
            const { data: activeEvents } = await supabase
                ...
                .order('start_datetime', { ascending: true }) // Closest upcoming
                .limit(1);
...
<div class="flex flex-1 flex-col gap-4 p-4 pt-0">
	...
	<Card class="relative overflow-hidden border-border bg-linear-to-br from-card to-card/50 shadow-lg min-h-50">
		...
		<Card>
			...
			<CardContent>
				...
				<div class="mt-1 h-1 w-full rounded-full bg-secondary">
                    <div class="h-1 rounded-full bg-green-500" style="width: {stats.total > 0 ? (stats.present / stats.total) * 100 : 0}%"></div>
                </div>
			</CardContent>
		</Card>
		<Card>
			...
			<CardContent>
                ...
                <div class="mt-1 h-1 w-full rounded-full bg-secondary">
                    <div class="h-1 rounded-full bg-red-500" style="width: {stats.total > 0 ? (stats.absent / stats.total) * 100 : 0}%"></div>
                </div>
			</CardContent>
		</Card>
	</div>
